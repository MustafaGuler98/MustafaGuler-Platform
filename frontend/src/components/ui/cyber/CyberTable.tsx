    ...
    return (
        <div className="w-full overflow-x-auto">
            <table className="w-full border-collapse">
                <thead>
                    <tr className="border-b border-white/10 bg-violet-950/30">
                        {columns.map((col) => (
                            <th
                                key={String(col.key)}
                                className="text-left py-4 px-4 font-mono text-xs font-bold tracking-widest text-violet-300 uppercase"
                            >
                                {col.label}
                            </th>
                        ))}
                    ...
                    {data.map((row, rowIndex) => (
                        <tr
                            ...
                            className={cn(
                                ...
                                'hover:bg-violet-500/5 hover:border-y-violet-500/30',
                                'border-l-2 border-l-white/[0.03] hover:border-l-violet-500',
                                'cursor-pointer'
                            )}
                        >
                            ...
                            {columns.map((col) => (
                                <td
                                    ...
                                    className={cn(
                                        ...
                                        "group-hover:before:content-[''] group-hover:before:absolute group-hover:before:top-0 group-hover:before:left-0 group-hover:before:right-0 group-hover:before:h-[1px] group-hover:before:bg-violet-500/30"
                                    )}
                                >
                                    {col.render
                                        ? col.render(row)
