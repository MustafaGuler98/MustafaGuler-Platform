...
import { Hourglass, Calendar, Tag, ArrowRight, ArrowUpCircle, Search } from "lucide-react";
...
export default function TimelineClient({ initialArticles }: TimelineClientProps) {
  ...
  const filteredArticles = initialArticles.filter((article) => {
    // Search Query Check (Title or Category)
  ...
  return (
    <div className="container max-w-4xl mx-auto">
        ...
        <div className="flex flex-col items-center justify-center mb-8 text-center">
           {/* Removed complex animations to fix Hydration Error */}
           <h1 className="text-5xl md:text-6xl font-bold font-heading mb-4 flex items-center gap-6 
                          bg-gradient-to-r from-primary via-purple-400 to-secondary bg-clip-text text-transparent
                          drop-shadow-[0_0_10px_rgba(168,85,247,0.3)] select-none">
             <Hourglass className="w-10 h-10 text-primary rotate-180" />
             TIMELINE
             <Hourglass className="w-10 h-10 text-primary" />
           </h1>
           
           <p className="text-muted-foreground font-sans text-lg tracking-wide border-b border-border/30 pb-2 mb-6">
             Archive Logs
           </p>
           ...
           <div className="relative w-full max-w-md">
             ...
             <input 
               ...
               className="w-full bg-background/50 border border-primary/30 rounded-full py-2 pl-10 pr-4 
        ...
        {/* CONTENT */}
        {/* Tightened Spacing: Reduced margin-left and space-y */}
        <div className="relative border-l-2 border-border/40 ml-4 md:ml-12 space-y-8 min-h-[300px]">
          
          {years.length === 0 ? (
             <div className="pl-12 pt-10 text-muted-foreground font-mono">
            ...
            years.map((year) => (
              <div key={year} className="relative">
                ...
                {/* YEAR CONTENT */}
                {/* Reduced padding-left (pl-12) and negative margin (-mt-20) to pull content up */}
                <div className="pl-8 md:pl-16 -mt-20">
                  {Object.keys(grouped[year]).map((month) => (
                    <div key={month} className="mb-6 last:mb-0">
                      
                      {/* MONTH HEADER */}
                      <h3 className="text-lg font-bold font-magic text-muted-foreground mb-4 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_10px_var(--color-primary)]" />
                        {month}
                      </h3>
                      ...
                      <div className="space-y-3">
                        {grouped[year][month].map((article) => (
                          ...
                          >
                            ...
                            {/* COMPACT CARD */}
                            {/* Reduced padding (p-3), flex-row layout for compactness */}
                            <div className="bg-card border border-border rounded-md p-3 relative overflow-hidden transition-all duration-300
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                  ...
                                  <div className="flex flex-col gap-1">
                                      <h4 className="text-base font-bold font-heading text-foreground group-hover:text-red-500 transition-colors leading-tight">
                                        {article.title}
                                      </h4>
