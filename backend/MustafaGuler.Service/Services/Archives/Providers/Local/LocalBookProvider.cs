...
{
    public class LocalBookProvider : IActivityProvider
    {
        private readonly IGenericRepository<Book> _repository;

        public LocalBookProvider(IGenericRepository<Book> repository)
        {
            _repository = repository;
        }

        public string ProviderType => "Local";
        public string ActivityType => "Book";

        public async Task<List<ActivityOptionDto>> GetRefreshedOptionsAsync()
        {
            var items = await _repository.GetAllAsync(x => !x.IsDeleted);
            return items.Select(b => new ActivityOptionDto
            {
                Id = b.Id,
                Title = b.Title,
