...
{
    public class LocalTvSeriesProvider : IActivityProvider
    {
        private readonly IGenericRepository<TvSeries> _repository;

        public LocalTvSeriesProvider(IGenericRepository<TvSeries> repository)
        {
        ...
        public async Task<PublicActivityDto?> GetDetailsAsync(Guid itemId)
        {
            ...
            return new PublicActivityDto
            {
                Type = "TvSeries",
                Title = item.Title,
                Subtitle = item.StartYear?.ToString(),
