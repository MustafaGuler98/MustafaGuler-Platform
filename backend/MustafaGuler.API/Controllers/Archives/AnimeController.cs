...
{
    [Route("api/archives/anime")]
    [ApiController]
    public class AnimeController : CustomBaseController
    {
        private readonly IAnimeService _service;

        public AnimeController(IAnimeService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Add([FromBody] CreateAnimeDto dto)
        {
        ...
        [Authorize(Roles = "Admin")]
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] UpdateAnimeDto dto)
        {
