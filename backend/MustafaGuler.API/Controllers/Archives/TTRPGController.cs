...
{
    [Route("api/archives/ttrpg")]
    [ApiController]
    public class TTRPGController : CustomBaseController
    {
        private readonly ITTRPGService _service;

        public TTRPGController(ITTRPGService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Add([FromBody] CreateTTRPGDto dto)
        {
        ...
        [Authorize(Roles = "Admin")]
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] UpdateTTRPGDto dto)
        {
